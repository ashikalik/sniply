...
export class ListQrCodesV1Service {
  ...
  async list(start?: number, page?: number, userId?: string) {
    ...
    const safeStart = Number.isFinite(start) && (start as number) >= 0 ? Number(start) : 0;
    const safePage = Number.isFinite(page) && (page as number) > 0 ? Math.min(Number(page), 100) : 10;
